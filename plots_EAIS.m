...
cd('O:\Documents\RATES\R Code\img\zwally\EGU\paper')
R_new28 = xlsread('results_EAISpaper_zw28.xls');
n = 1:10; 
n3 = repmat(n,1,11); 
b=zeros(25,1);c2=[n3';b];
...
T3_smb=Rcm4_res(find(Rcm4_res(:,1)==3),5:6); sig3_ice=[T3_smb(:,1)+T3_smb(:,2), T3_smb(:,1)-T3_smb(:,2)];
T3_ice=Rcm4_res(find(Rcm4_res(:,1)==4),5:6); sig3_ice=[T3_ice(:,1)+T3_ice(:,2), T3_ice(:,1)-T3_ice(:,2)];

T2_smb28=R_new28(find(R_new28(:,1)==4),5:6); sig2_ice=[T2_smb28(:,1)+T2_smb28(:,2), T2_smb28(:,1)-T2_smb28(:,2)];
T2_ice28=R_new28(find(R_new28(:,1)==5),5:6); sig2_ice=[T2_ice28(:,1)+T2_ice28(:,2), T2_ice28(:,1)-T2_ice28(:,2)];
...
hold on
h1=plot(2003:2013,T1_smb(:,1),'b','LineWidth',2);
h2=plot(2003:2013,T2_smb(:,1),'--r','LineWidth',2);
h3=plot(2003:2013,T3_smb(:,1),':g','LineWidth',4);
...
subplot(1,2,2)
plotshaded(2003:2013,sig1_ice,'cy');
hold on
h3=plot(2003:2013,T1_ice(:,1),'b','LineWidth',2);
plotshaded(2003:2013,sig2_ice,'m');
h4=plot(2003:2013,T2_ice(:,1),'--r','LineWidth',2);
plotshaded(2003:2013,sig3_ice,'y');
h5=plot(2003:2013,T3_ice(:,1),':g','LineWidth',4);
...
h3=plot(2003:2013,T3_smb(:,1),'*--k','LineWidth',3);
